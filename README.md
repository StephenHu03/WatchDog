...
> 本文档是给 Codex / 开发工程师直接执行的技术实现规格，不是产品宣传文档。
>
> 目标：实现一个 Windows Python GUI EXE 工具，对 PHP 后端项目进行离线授权绑定、授权核心保护和交付打包。
>
> 核心规则：**域名 OR IPv4 任一匹配即可运行；客户业务源码允许修改；授权核心必须受到保护；运行时不得请求厂商服务器；不得因授权系统改变正常 PHP 部署方式。**
